...
public class MediaJobs : MonoBehaviour
{
    ...
    private void LoadDropDown() {
        ...
        for (int i = 0; i < AD.CurrentPlayer.Skills.Media.Count; i++) {
            Debug.Log("dcascacsccacscscascasc");
            GameObject item = Instantiate(ItemPrefab, Content);
            //item.transform.GetChild(2).gameObject.SetActive(false);
            item.GetComponent<item5>().Title.text = AD.CurrentPlayer.Skills.Media[i].title;
            ...
            item.GetComponent<item5>().Btn.onClick.AddListener(() => {
                ...
                //    selection.ListName = AppData.Fields.Media;
                ...
                if (item.GetComponent<item5>().isSelected) {
                    ...
                    for (int k = 0; k < AD.SelectThree.Count; k++) {
                        if (AD.SelectThree[k].ListName == AppData.Fields.Media && AD.SelectThree[k].Listindex == item.GetComponent<item5>().index) {

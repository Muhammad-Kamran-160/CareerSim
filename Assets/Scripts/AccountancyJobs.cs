...
public class AccountancyJobs : MonoBehaviour
{
    ...
    private void LoadDropDown() {
        ...
        Debug.Log(AD.CurrentPlayer.Skills.Accountacy.Count);
        for (int i = 0; i < AD.CurrentPlayer.Skills.Accountacy.Count; i++) {
            Debug.Log("dcascacsccacscscascasc");
            GameObject item = Instantiate(ItemPrefab, Content);
           // item.transform.GetChild(2).gameObject.SetActive(false);
            item.GetComponent<item5>().Title.text = AD.CurrentPlayer.Skills.Accountacy[i].title;
            ...
            item.GetComponent<item5>().Btn.onClick.AddListener(() => {
                ...
                if (item.GetComponent<item5>().isSelected) {
                    ...
                    selection.ListName = AppData.Fields.Accountancy;
                    ...
                    for (int k = 0; k < AD.SelectThree.Count; k++) {
                        if(AD.SelectThree[k].ListName == AppData.Fields.Accountancy && AD.SelectThree[k].Listindex == item.GetComponent<item5>().index) {
